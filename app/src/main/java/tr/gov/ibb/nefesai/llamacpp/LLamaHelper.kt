...
class LlamaHelper(
    ...
    private val llamaDispatcher = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "LlamaInferenceThread").apply {
            android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO)
        }
    }.asCoroutineDispatcher()

    val scope: CoroutineScope = CoroutineScope(llamaDispatcher)
            ...
            loadJob = scope.launch {
                val result = try {
                    llama.startEngine(config) {
                        allText += it
                        tokenCount++
                        // tryEmit yerine emit veya doğrudan akış kontrolü
                ...
                val id = result["contextId"] ?: throw Exception("contextId not found")
